 ...
 */
class GroupMenuLinkEditAccessCheck implements AccessInterface {
  ...
  public function access(Route $route, RouteMatch $route_match, AccountInterface $account) {
    ...
    if (!$route->getRequirement('_group_menu_link_edit_access') === 'TRUE') {
      return AccessResult::allowed();
    }
    
    // Get the menu this link belongs to
    $menu = Menu::load($route_match->getParameter('menu_link_content')->getMenuName());
    ...
    foreach($group_menus as $group_menu) {
      ...
      if ($access = $group->hasPermission('edit group menus', $account)) {
        return AccessResult::allowed();      
      }
    }
